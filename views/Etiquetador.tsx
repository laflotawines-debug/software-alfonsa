...
export const Etiquetador: React.FC = () => {
    ...
    const addSelectedToQueue = () => {
        ...
        if (toAdd.length === 0 && selectedFromSearch.size > 0) {
            alert("Los artículos seleccionados ya se encuentran en la cola de impresión.");
    ...
    return (
        <div className="flex flex-col lg:flex-row h-auto lg:h-[calc(100vh-140px)] gap-6 animate-in fade-in overflow-y-auto lg:overflow-hidden pb-10 lg:pb-0">
            ...
            <div className="flex-1 flex flex-col gap-6 min-w-0">
                ...
                <div className="bg-surface rounded-3xl border border-surfaceHighlight shadow-sm flex flex-col shrink-0 overflow-hidden">
                    <div className="p-4 md:p-5 border-b border-surfaceHighlight flex flex-col gap-4 bg-background/20">
                        <div className="flex flex-col md:flex-row gap-3">
                            <div className="relative flex-1">
                                <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-muted" size={18} />
                                <input 
                                    type="text" placeholder="Búsqueda inteligente..." value={searchTerm}
                        ...
                        <div className="flex flex-col sm:flex-row items-center justify-between gap-3 border-t border-surfaceHighlight/30 pt-3">
                            <div className="w-full sm:w-auto flex items-center gap-2 bg-background px-4 py-2 rounded-2xl border border-surfaceHighlight">
                            ...
                            <button 
                                ...
                                className="w-full sm:w-auto flex items-center justify-center gap-2 px-8 py-3 bg-primary text-white rounded-2xl font-black text-xs uppercase shadow-lg disabled:opacity-30 transition-all active:scale-95"
                            >
                    ...
                    <div className="overflow-x-auto max-h-[400px] lg:max-h-[550px]">
                        <table className="w-full text-left">
                            <thead className="bg-background/40 text-[9px] text-muted uppercase font-black tracking-widest border-b border-surfaceHighlight sticky top-0 z-10">
                                <tr>
                                    ...
                                    <th className="p-3 text-center hidden sm:table-cell">Estado</th>
                                </tr>
                            </thead>
                                    ...
                                    <tr key={p.codart} className={`group hover:bg-primary/5 transition-colors cursor-pointer ${selectedFromSearch.has(p.codart) ? 'bg-primary/5' : ''}`} onClick={() => toggleSearchSelection(p.codart)}>
                                        ...
                                        <td className="p-3 text-right">
                                            <div className="flex flex-col items-end">
                                                <span className={`text-xs font-black ${p.isEndingOffer ? 'text-primary' : 'text-text'}`}>$ {p.pventa_1.toLocaleString('es-AR')}</span>
                                                {p.currentItemDiscount > 0 && <span className="text-[9px] text-primary font-bold">-{p.currentItemDiscount}%</span>}
                                            </div>
                                        </td>
                                        <td className="p-3 text-center hidden sm:table-cell">
                                            {p.isNew ? <span className="px-2 py-0.5 rounded bg-blue-500/10 text-blue-600 border border-blue-200 text-[8px] font-black uppercase">Nueva</span> : p.isChange ? <span className="px-2 py-0.5 rounded bg-orange-500/10 text-orange-600 border border-orange-200 text-[8px] font-black uppercase">Cambio</span> : <span className="px-2 py-0.5 rounded bg-green-500/10 text-green-600 border border-green-200 text-[8px] font-black uppercase">OK</span>}
                                        </td>
                                    </tr>
                                ))}
                            </tbody>
                ...
                {/* COLA DE IMPRESIÓN */}
                <div className="bg-surface rounded-3xl border border-surfaceHighlight shadow-sm flex flex-col overflow-hidden min-h-[300px] lg:flex-1 lg:min-h-0">
                            ...
                            <tbody className="divide-y divide-surfaceHighlight">
                                {labelQueue.map((item) => (
                                    <tr key={item.id} className={`group hover:bg-primary/5 transition-colors ${!item.selected ? 'opacity-40 grayscale' : ''}`}>
                                        ...
                                        <td className="p-4 text-right"><p className="text-sm font-black text-green-600">$ {Math.round(item.finalPrice).toLocaleString('es-AR')}</p></td>
            ...
            {/* SIDEBAR DERECHO (DISEÑO Y AJUSTES) */}
            <div className="w-full lg:w-[320px] xl:w-[380px] flex flex-col gap-6 shrink-0 lg:h-full lg:overflow-y-auto lg:pr-2 lg:pb-10 scrollbar-thin">
                <div className="bg-surface rounded-3xl border border-surfaceHighlight shadow-sm flex flex-col shrink-0">
                    ...
                    <div className="p-6 lg:p-8 bg-background/40 flex justify-center items-center min-h-[200px]">
                ...
                <div className="bg-surface rounded-3xl border border-surfaceHighlight p-5 lg:p-6 shadow-sm shrink-0">
                ...
                <div className="grid grid-cols-2 gap-3 px-1 lg:px-0">
