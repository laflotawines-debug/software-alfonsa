...
export const OrderAssemblyModal: React.FC<OrderAssemblyModalProps> = ({
    ...
    const buildInvoicePDF = () => {
        ...
        const delivered = order.products.filter(p => p.quantity > 0).sort((a, b) => a.name.localeCompare(b.name));
        ...
        }).filter(p => p.missing > 0).sort((a, b) => a.name.localeCompare(b.name));
        ...
        const returns = order.products.map(p => {
             ...
             return { ...p, returned };
        }).filter(p => p.returned > 0).sort((a, b) => a.name.localeCompare(b.name));
    ...
    const canDelete = isVale || (currentUser.permissions || []).includes('orders.create');
                ...
                <>
                    ...
                    {canDelete && (
                        <button 
                            ...
                            className="w-full py-3 rounded-xl bg-red-500/5 text-red-500 font-black border border-red-500/10 hover:bg-red-500 hover:text-white transition-all text-[10px] uppercase flex items-center justify-center gap-2 mt-2"
                        >
                            <Trash2 size={14} />
                            Eliminar Pedido
                        </button>
